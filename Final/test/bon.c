...
int main()
{
	...
	do {
        ...
        printf("Distance Pomme : %d", CheminDirectPomme);
        gotoxy(2+LARGEUR_PLATEAU, 2);
        printf("Distance Haut : %d", CheminPortailHaut);
        gotoxy(2+LARGEUR_PLATEAU, 3);
        printf("Distance Bas : %d", CheminPortailBas);
        gotoxy(2+LARGEUR_PLATEAU, 4);
        printf("Distance Gauche : %d", CheminPortailGauche);
        gotoxy(2+LARGEUR_PLATEAU, 5);
        printf("Distance Droit : %d", CheminPortailDroite);
...
// Fonction pour vérifier si une position donnée est occupée par le corps du serpent
...
// Fonction pour vérifier si une direction est sûre (pas de collision avec le corps)
bool directionEstSure(int x, int y, char direction, int lesX[], int lesY[], tPlateau plateau) {
    ...
    if (nextX <= 0) nextX = LARGEUR_PLATEAU-1;
    else if (nextX > LARGEUR_PLATEAU) nextX = 2;
    if (nextY <= 0) nextY = HAUTEUR_PLATEAU;
    else if (nextY > HAUTEUR_PLATEAU) nextY = 1;
    
    return !estSurCorpsSerpent(nextX, nextY, lesX, lesY) && plateau[nextX][nextY] != BORDURE;
}

// Fonction pour trouver une direction alternative sûre
char trouverDirectionSure(int lesX[], int lesY[], char directionActuelle, tPlateau plateau) {
    ...
    // D'abord, vérifier si la direction actuelle est sûre
    if (directionEstSure(lesX[0], lesY[0], directionActuelle, lesX, lesY, plateau)) {
        return directionActuelle;
    }
    
    // Sinon, essayer les autres directions
    for (int i = 0; i < 4; i++) {
        if (directions[i] != directionActuelle && 
            directionEstSure(lesX[0], lesY[0], directions[i], lesX, lesY, plateau)) {
            return directions[i];
        }
    }
    
    // Si aucune direction n'est sûre, retourner la direction actuelle
    return directionActuelle;
}
