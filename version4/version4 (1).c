...
#define Y_DEPART_SERPENT 13  // Position en Y du serpent au départ (au centre du plateau).
#define X_DEPART_SERPENT_2 40  // Position en X du serpent au départ (au centre du plateau).
#define Y_DEPART_SERPENT_2 27  // Position en Y du serpent au départ (au centre du plateau).
...
void progresser2(int lesX2[], int lesY2[], char direction, tPlateau plateau, bool *collision, bool *pomme); // Fait avancer le serpent dans une direction donnée.
...
int main() {
    ...
    do {
        ...
        int CheminDirectPomme2 = abs(lesX2[0] - lesPommesX[NbPommesSerpentManger]) + abs(lesY2[0] - lesPommesY[NbPommesSerpentManger]); // Calcul de la distance directe entre la tête du serpent et la pomme
        int CheminPortailHaut2 = abs(lesX2[0] - TROU_HAUT.x) + abs(lesY2[0] - TROU_HAUT.y) + abs(lesPommesX[NbPommesSerpentManger] - TROU_BAS.x) + abs(lesPommesY[NbPommesSerpentManger] - TROU_BAS.y);
        int CheminPortailBas2 = abs(lesX2[0] - TROU_BAS.x) + abs(lesY2[0] - TROU_BAS.y) + abs(lesPommesX[NbPommesSerpentManger] - TROU_HAUT.x) + abs(lesPommesY[NbPommesSerpentManger] - TROU_HAUT.y);
        int CheminPortailGauche2 = abs(lesX2[0] - TROU_GAUCHE.x) + abs(lesY2[0] - TROU_GAUCHE.y) + abs(lesPommesX[NbPommesSerpentManger] - TROU_DROITE.x) + abs(lesPommesY[NbPommesSerpentManger] - TROU_DROITE.y);
        int CheminPortailDroite2 = abs(lesX2[0] - TROU_DROITE.x) + abs(lesY2[0] - TROU_DROITE.y) + abs(lesPommesX[NbPommesSerpentManger] - TROU_GAUCHE.x) + abs(lesPommesY[NbPommesSerpentManger] - TROU_GAUCHE.y);
        ...
        switch (minimunDes5distances2)
        {
            case 0:
                // Cas 0 : Chemin direct vers la pomme
                if ((lesPommesX[NbPommesSerpentManger] - lesX2[0]) < 0) 
                {
                    direction2 = GAUCHE; // La pomme est à gauche de la tête du serpent.
                } 
                else if ((lesPommesX[NbPommesSerpentManger] - lesX2[0]) > 0) 
                {
                    direction2 = DROITE; // La pomme est à droite de la tête du serpent.
                } 
                else if ((lesPommesY[NbPommesSerpentManger] - lesY2[0]) < 0) 
                {
            ...
            case 1:
                ...
                if (PasserPortails(lesX2, lesY2) == false)
                {
                    ...
                    {
                        direction2 = GAUCHE; // Le portail haut est à gauche de la tête.
                    } 
            ...
            case 2:
                ...
                if (PasserPortails(lesX2, lesY2) == false) 
                {
                    ...
                    {
                        direction2 = GAUCHE; // Le portail bas est à gauche de la tête.
                    } 
            ...
            case 3:
                ...
                if (PasserPortails(lesX2, lesY2) == false) 
                {
                    ...
                    {
                        direction2 = GAUCHE; // Le portail gauche est à gauche de la tête.
                    } 
            ...
            case 4:
                ...
                if (PasserPortails(lesX2, lesY2) == false)
                {
                    ...
                    {
                        direction2 = GAUCHE; // Le portail droit est à gauche de la tête.
                    } 
	...
	if (gagne)
	{
		...
		printf("Compteur de pas 2 : %d\n", nbDepUnitaires2);// Affiche les performances du programme.
	}
	return EXIT_SUCCESS;
}
...
void progresser2(int lesX2[], int lesY2[], char direction2, tPlateau plateau, bool *collision, bool *pomme) {   // Met à jour la position du serpent
    direction2 = trouverDirectionSure(lesX2, lesY2, direction2, plateau); // Vérifie ou trouve une direction sûre
    effacer(lesX2[TAILLE - 1], lesY2[TAILLE - 1]); // Efface le dernier segment du serpent

    for (int i = TAILLE - 1; i > 0; i--)// Déplace chaque segment vers l'avant 
    { 
        lesX2[i] = lesX2[i - 1]; // Copie la position x du segment précédent
        lesY2[i] = lesY2[i - 1]; // Copie la position y du segment précédent
    }
    switch (direction2) // Met à jour la position de la tête selon la direction
    { 
        case HAUT: 
            lesY2[0]--; // Monte
            break;                                                                       
    ...
    if (lesX2[0] <= 0)
    {
        lesX2[0] = LARGEUR_PLATEAU;// Gère la sortie à gauche
    }                                                          
    else if (lesX2[0] > LARGEUR_PLATEAU)
    {
        lesX2[0] = 1; // Gère la sortie à droite
    } 

    if (lesY2[0] <= 0) 
    {
        lesY2[0] = HAUTEUR_PLATEAU;// Gère la sortie en haut
    }                                                          
    else if (lesY2[0] > HAUTEUR_PLATEAU)
    {
        lesY2[0] = 1; // Gère la sortie en bas 
    }
    
    *pomme = (plateau[lesX2[0]][lesY2[0]] == POMME); // Vérifie si une pomme est mangée
    if (*pomme) 
    {
        plateau[lesX2[0]][lesY2[0]] = VIDE;   // Supprime la pomme si mangée
    }
    ...
    if (estSurCorpsSerpent(lesX2[0], lesY2[0], lesX2, lesY2)) 
    {
        *collision = true;// Vérifie la collision avec le corps  
    }                         
    dessinerSerpent2(lesX2, lesY2); // Redessine le serpent
    nbDepUnitaires2++; // Incrémente le compteur de mouvements
}
