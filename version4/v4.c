...
#define Y_DEPART_SERPENT 14  // Position en Y du serpent au départ (au centre du plateau).
#define X_DEPART_SERPENT_2 40  // Position en X du serpent au départ (au centre du plateau).
#define Y_DEPART_SERPENT_2 26  // Position en Y du serpent au départ (au centre du plateau).
    ...
    do {
        ...
        switch (minimunDes5distances2)
        {
            case 0:
                // Cas 0 : Chemin direct vers la pomme
                // Si la pomme est plus proche sans utiliser de portail, on va vers la pomme.
                if ((lesPommesY[NbPommesSerpentManger] - lesY2[0]) < 0) 
                {
            ...
            case 2:
                ...
                {
                    // Si le serpent n'est pas encore au portail bas, il se dirige vers le portail.
                    if ((TROU_BAS.y - lesY2[0]) < 0) 
                    {
                        direction2 = HAUT; // Le portail bas est au-dessus de la tête du serpent.
                    } 
            ...
            case 3:
                ...
                {
                    // Si le serpent n'est pas encore au portail gauche, il se dirige vers le portail.
                    if ((TROU_GAUCHE.y - lesY2[0]) < 0) 
                    {
                        direction2 = HAUT; // Le portail gauche est au-dessus de la tête du serpent.
                    } 
            ...
            case 4:
                ...
                {
                    // Si le serpent n'est pas encore au portail droit, il se dirige vers le portail.
                    if ((TROU_DROITE.y - lesY2[0]) < 0) 
                    {
                        direction2 = HAUT; // Le portail droit est au-dessus de la tête du serpent.
                    } 
		...
		if (pommeMangee || pommeMangee2) // Ajoute une pomme au compteur de pommes quand elle est mangée et arrête le jeu si le score atteint 10.
		{
            if (pommeMangee){
...
void progresser(int lesX[], int lesY[], char direction, tPlateau plateau, bool *collision, bool *pomme, int autreX[], int autreY[]) {   
    ...
    *pomme = (plateau[lesX[0]][lesY[0]] == POMME);
    ...
    if (estSurCorpsSerpent(lesX[0], lesY[0], lesX, lesY) || 
        estSurCorpsAutreSerpent(lesX[0], lesY[0], autreX, autreY)) 
    {
        *collision = true;
    }                         
...
void progresser2(int lesX2[], int lesY2[], char direction2, tPlateau plateau, bool *collision, bool *pomme, int autreX[], int autreY[]) {   
    ...
    *pomme = (plateau[lesX2[0]][lesY2[0]] == POMME);
    ...
    if (estSurCorpsSerpent(lesX2[0], lesY2[0], lesX2, lesY2) || 
        estSurCorpsAutreSerpent(lesX2[0], lesY2[0], autreX, autreY)) 
    {
        *collision = true;
    }                         
