...
void disable_echo();
void enable_echo();
char determinerDirection(int xTete, int yTete, int xPomme, int yPomme);
...
int main(){
    ...
    do {
        // Déterminer la direction vers la prochaine pomme
        direction = determinerDirection(lesX[0], lesY[0], lesPommesX[nbPommes], lesPommesY[nbPommes]);
        progresser(lesX, lesY, direction, lePlateau, &collision, &pommeMangee);
        if (pommeMangee){
            nbPommes++;
            gagne = (nbPommes == NB_POMMES);
            if (!gagne){
                ajouterPomme(lePlateau, lesPommesX[nbPommes], lesPommesY[nbPommes]);
                pommeMangee = false;
            }
        }
        if (!gagne){
...
char determinerDirection(int xTete, int yTete, int xPomme, int yPomme){
    if (xTete < xPomme) return DROITE;
    if (xTete > xPomme) return GAUCHE;
    if (yTete < yPomme) return BAS;
    if (yTete > yPomme) return HAUT;
    return direction;
}
