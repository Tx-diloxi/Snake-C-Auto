...
int choisirDirection(int X, int Y, tPlateau lePlateau) {
    ...
    int cheminTrouD = ( abs( LARGEUR_PLATEAU - X) + abs( (HAUTEUR_PLATEAU / 2) - Y) +  abs( 1 - lesPommesX[nbPommes]) + abs( (HAUTEUR_PLATEAU / 2) - lesPommesX[nbPommes]) );

    int cheminTrouG = ( abs( 1 - X) + abs( (HAUTEUR_PLATEAU / 2) - Y) +  abs( LARGEUR_PLATEAU - lesPommesX[nbPommes]) + abs( (HAUTEUR_PLATEAU / 2) - lesPommesX[nbPommes]) );

    int cheminTrouH = ( abs( ( LARGEUR_PLATEAU / 2 ) - X) + abs( 1 - Y) +  abs( ( LARGEUR_PLATEAU / 2 ) - lesPommesX[nbPommes]) + abs( (HAUTEUR_PLATEAU / 2) - lesPommesX[nbPommes]) );

    int cheminTrouB = ( abs( ( LARGEUR_PLATEAU / 2 ) - X) + abs( HAUTEUR_PLATEAU - Y) +  abs( ( LARGEUR_PLATEAU / 2 ) - lesPommesX[nbPommes]) + abs( (1) - lesPommesX[nbPommes]) );
