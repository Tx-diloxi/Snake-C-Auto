...
void determinerDirectionVersObjectif(int lesX[], int lesY[], tPlateau plateau, char *direction, int objectifX, int objectifY);
...
int main()
{
    ...
    do
	{
		...
		if (pommeMangee)
		{
			...
			SerpentPasserPortail = false; // remet en false pour pouvoir se retéléporter après avoir manger une pomme
			if (!gagne)
			{
				ajouterPomme(lePlateau);
				meilleurDistance = DistancePlusCourt(lesX, lesY, lesPommesX[NbPommesManger], lesPommesY[NbPommesManger]); // recalcul la meilleur position après l'apparition d'une nouvelle pomme
				pommeMangee = false;
			}
		}
		if (!gagne)
 ...
 * @param SerpentPasserPortail de type bool, vérifie si le serpent s'est téléporter
 */
void progresser(int lesX[], int lesY[], char direction, tPlateau plateau, bool *collision, bool *pomme, bool *SerpentPasserPortail)
{
	...
	// Gestion des téléportations du serpent
	if (lesX[0] <= 0)
	{
		lesX[0] = LARGEUR_PLATEAU; // faire apparaitre à gauche
		*SerpentPasserPortail = true;		   // quand le serpent traverse le trou
	}
